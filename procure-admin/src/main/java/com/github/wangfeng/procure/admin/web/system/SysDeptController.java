...
import com.github.wangfeng.procure.common.po.dto.SysDeptDTO;
...
import com.github.wangfeng.procure.service.system.SysDeptService;
...
public class SysDeptController extends BaseController {
    ...
    public List<SysDeptDO> list(SysDeptDO dept) {
        List<SysDeptDO> deptList = deptService.selectDeptList(dept);
        return deptList;
    }
    ...
    @PostMapping("/add")
    @ResponseBody
    public AjaxResult addSave(@Validated SysDeptDO dept) {
    ...
    public String edit(@PathVariable("deptId") Long deptId, ModelMap mmap) {
        SysDeptDO deptDO = deptService.selectDeptById(deptId);
        SysDeptDTO dept = new SysDeptDTO();
        BeanUtils.copyProperties(deptDO, dept);
        if (StringUtils.isNotNull(deptDO) && 100L == deptId) {
    ...
    @PostMapping("/edit")
    @ResponseBody
    public AjaxResult editSave(@Validated SysDeptDO dept) {
        if (UserConstants.DEPT_NAME_NOT_UNIQUE.equals(deptService.checkDeptNameUnique(dept))) {
    ...
    @ResponseBody
    public String checkDeptNameUnique(SysDeptDO dept) {
    ...
    public List<Ztree> treeData() {
        List<Ztree> ztrees = deptService.selectDeptTree(new SysDeptDO());
        return ztrees;
    }
    ...
    @ResponseBody
    public List<Ztree> deptTreeData(SysRoleDO role) {
